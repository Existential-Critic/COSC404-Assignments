...
public class QueryMongoMapReduce {
    ...
    public MongoCursor<Document> query2() {
		...
		String mapfn = """
			function() {
				emit(\"totalOfAllOrders\", this.orders);
			};
		""";
		String reducefn = """
			function(key, items) {
    ...
    public MongoCursor<Document> query4() {
		...
		String reducefn = """
			function(key, items) {
				...
				for(i = 0;i < items.length;i++) {
					if(items[i].name > \"K\") {
						for(j = 0;j < items[i].orders.length;j++) {
							reducedObject.fCount += items[i].orders[j].items;
							reducedObject.tCount++;
						}
					}
				}
		...
		String finalisefn = """
			function(key, reducedValue) {
				if(reducedValue.tCount == 0) {
					return 0;
				}else {
					return reducedValue.fCount/reducedValue.tCount;
				}
			};
		""";

		MongoCollection<Document> col = db.getCollection(COLLECTION_NAME);
		MapReduceIterable<Document> output = col.mapReduce(mapfn, reducefn).finalizeFunction(finalisefn);
		return output.iterator();
    }
    ...
    public MongoCursor<Document> query5() {
		...
		String reducefn = """
			function(key, items) {
				...
				for(i = 0;i < items.length;i++) {
					for(j = 0;j < items[i].length;j++) {
						if(items[i][j].items > maxItem) {
							maxItem = items[i][j].items;
							iKey = i;
							jKey = j;
						}
					}
				}
				return items[iKey][jKey];
			};
		""";
		MongoCollection<Document> col = db.getCollection(COLLECTION_NAME);
		MapReduceIterable<Document> output = col.mapReduce(mapfn, reducefn);
    ...
    public MongoCursor<Document> query6() {
		...
		String mapfn = """
			function() {
				emit(\"firstlonger\",this);
				emit(\"firstshorter\",this);
				emit(\"same\",this);
			};
		""";
		String reducefn = """
			function(key, items) {
				...
				if(key == \"firstlonger\") {
					for(i = 0;i < items.length;i++) {
						nameArray = items[i].name.split(\" \");
						if(nameArray[0].length > nameArray[1].length) {
							nameCount++;
						}
					}
				}else if(key == \"firstshorter\") {
					for(i = 0;i < items.length;i++) {
						nameArray = items[i].name.split(\" \");
						if(nameArray[0].length < nameArray[1].length) {
							nameCount++;
