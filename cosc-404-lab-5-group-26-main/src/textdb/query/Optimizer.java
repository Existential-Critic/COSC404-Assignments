...
{	
	...
	{	SimpleNode parseRoot = buildParseTree(sql);
		...
		if(obNode != null) {
			// Determining if attribute is in the scheme 
			SimpleNode attributeNode = (SimpleNode) obNode.jjtGetChild(0);
			attributeName = extractIdentifier(attributeNode.toString());
		
			// Lookup table in schema
			Relation r3 = schema.getTable(tableName);
			if (r3.findAttributeByName(attributeName) == null)
				throw new SQLException("Attribute: "+attributeName+" not found in schema.");
			
			// Determining if ASC or DESC is specified 
			if(obNode.jjtGetNumChildren() >= 2) {
				if(obNode.jjtGetChild(1).toString().equals("DESC")) {
					ascOrder = false;
				}
			}

			Relation inputRelation = current.getOutputRelation();
			Attribute attribute = inputRelation.findAttributeByName(attributeName);
			int fieldIndex = inputRelation.getAttributeIndex(attribute);

			System.out.println("Attribute " + attributeName);
			System.out.println("Attribute Index " + fieldIndex);
			System.out.println("ascOrder " + ascOrder);

			SortComparator sorter = new SortComparator(new int[]{fieldIndex}, new boolean[]{ascOrder});
			textdb.operators.MergeSort mergeSortOp = new MergeSort(current, 10000, 100, sorter);
			mergeSortOp.setOutputRelation(inputRelation);
			current = mergeSortOp;
		}
